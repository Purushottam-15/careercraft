...
function displayStudents() {
  ...
  if (allStudents.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="9" style="text-align:center;">No students registered yet</td></tr>';
    return;
  }
...
function displayEmployers() {
  ...
  if (allEmployers.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align:center;">No employers registered yet</td></tr>';
    return;
  }

  allEmployers.forEach((employer) => {
    const jobsCount = allJobs.filter(
      (j) => j.employerId === employer.id,
    ).length;
    const row = document.createElement("tr");
    row.innerHTML = `
            <td>${employer.id}</td>
...
function displayApplications() {
  ...
  if (allApplications.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align:center;">No applications submitted yet</td></tr>';
    return;
  }
  ...
  allApplications.forEach((app) => {
    ...
    const employer = job
      ? allEmployers.find((e) => e.id === job.employerId)
      : null;
